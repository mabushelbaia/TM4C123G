...
int main(void)
{
    ...
    LCD4bits_Cmd(cursor1);                   // Force the cursor to beginning of 1st line
    ...
    LCD4bits_Cmd(cursor2);                   // Force the cursor to beginning of 1st line
    delayMs(500);                         // delay 500ms for LCD (MCU is faster than LCD)
...
{
    if (GPIOF->MIS & SW1 || GPIOF->MIS & SW2)
    { 
        unsigned char displacment = 0;
        while (True)
        {
            LCD4bits_Cmd(0x01); // Clear the display
            cursor1 = cursor + (((displacment++) + 9) % 16);
            cursor2 = cursor2 - (((displacment++) + 8) % 16);
            if cursor2 < 0x80
                cursor2 = 0xDF - cursor2;
            LCD4bits_Cmd(cursor1);  // Force the cursor to beginning of 1st line
            delayMs(500);          // delay 500ms for LCD (MCU is faster than LCD)
            LCD_WriteString(str1); // Write the string on LCD
            delayMs(500);          // Delay 500 ms to let the LCD diplays the data
